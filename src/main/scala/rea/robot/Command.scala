...
object Command {
  ...
  def apply(input: String): Option[Command] = input.trim.toUpperCase match {
    ...
    case placeCommandRegex(x, y, dir) => dir.trim.toUpperCase match {
      case "NORTH" => Some(Place(x.toInt, y.toInt, North))
      case "SOUTH" => Some(Place(x.toInt, y.toInt, South))
      case "EAST" => Some(Place(x.toInt, y.toInt, East))
      case "WEST" => Some(Place(x.toInt, y.toInt, West))
      case _ => None
    }
    case _ => None
  }
